...
export class TypeOrmVoucherAdapter implements BookingVoucherPort {
  ...
  private validate(
    voucher: Voucher,
    totalPrice: number,
  ): { id: string; discountAmount: number } {
    ...
    if (
      ...
      totalPrice < Number(voucher.minOrderValue)
    ) {
      throw new BadRequestException(
        `Don hang chua dat gia tri toi thieu de ap dung ma giam gia (${Number(voucher.minOrderValue).toLocaleString()} VND)`,
      );
    }
