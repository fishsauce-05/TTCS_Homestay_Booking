...
export interface BookingVoucherPort {
  validateById(voucherId: string, totalPrice: number): Promise<{ id: string; discountAmount: number }>;
  validateByCode(code: string, totalPrice: number): Promise<{ id: string; discountAmount: number }>;
