  ...
  async execute(query: ValidateVoucherQuery): Promise<{
    ...
    if (!voucher)
      throw new NotFoundException(
        'Ma giam gia khong hop le hoac khong ton tai',
      );

    this.eligibilityPolicy.assertEligible(voucher, query.totalPrice);

    return {
      valid: true,
      discountAmount: this.discountPolicy.calculate(voucher, query.totalPrice),
