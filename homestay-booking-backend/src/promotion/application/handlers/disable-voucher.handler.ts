...
import { DisableVoucherCommand } from '../commands/disable-voucher.command';
...
export class DisableVoucherHandler {
  ...
  async execute(command: DisableVoucherCommand): Promise<Voucher> {
    ...
    if (command.disabled === undefined) {
      voucher.status = voucher.status === VoucherStatus.ACTIVE ? VoucherStatus.INACTIVE : VoucherStatus.ACTIVE;
    } else {
      voucher.status = command.disabled ? VoucherStatus.INACTIVE : VoucherStatus.ACTIVE;
    }
