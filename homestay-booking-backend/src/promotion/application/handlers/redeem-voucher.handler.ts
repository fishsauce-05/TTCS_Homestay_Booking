...
import { RedeemVoucherCommand } from '../commands/redeem-voucher.command';
...
export class RedeemVoucherHandler {
  ...
  async execute(command: RedeemVoucherCommand): Promise<{
    ...
    const booking = await this.redemptions.findBookingById(command.dto.bookingId);
    ...
    const existing = await this.redemptions.findByBookingAndVoucher(booking.id, voucher.id);
    if (existing) {
