...
import { DisableVoucherCommand } from '../../application/commands/disable-voucher/disable-voucher.command';
import { RedeemVoucherCommand } from '../../application/commands/redeem-voucher/redeem-voucher.command';
import { UpdateVoucherCommand } from '../../application/commands/update-voucher/update-voucher.command';
...
import { GetVoucherDetailQuery } from '../../application/queries/get-voucher-detail/get-voucher-detail.query';
import { ListVouchersQuery } from '../../application/queries/list-vouchers/list-vouchers.query';
...
export class PromotionController {
  ...
  validateVoucher(@Body() body: { code: string; totalPrice: number }) {
    return this.validateVoucherHandler.execute(
      new ValidateVoucherQuery(body.code, body.totalPrice ?? 0),
    );
  }
  ...
  createVoucher(
    ...
    return this.createVoucherHandler.execute(
      new CreateVoucherCommand(dto, user.id),
    );
  }
